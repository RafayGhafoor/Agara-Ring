...
/// ring on Save. Only shown for devices that declare `.measurementInterval` (the ring, the ring); a
/// defensive empty-state covers the case of arriving here without that capability.
///
/// Note: on the ring only the HR interval maps to a command — SpO₂/stress/HRV/temperature ride its
/// combined-sensor packet and its background log, with no per-metric opcode to switch them off, so
/// those toggles are inert there. On an earlier family the same is true of **stress alone**: the ring derives it
/// from the HRV sweep and files it in the body-data record, and an earlier protocol setting catalog has no stress
